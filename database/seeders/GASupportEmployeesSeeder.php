...
class GASupportEmployeesSeeder extends Seeder
{
    ...
    public function run(): void
    {
        activity()->withoutLogs(function () {
            ...
            // Try to find GA Support job family with Company Driver job titles
            $jobFamily = JobFamily::with(['jobTitles' => function ($query) {
                $query->whereLike('job_title', "%Company Driver%");
            }])
                ->whereLike('job_family_name', "%General Affairs-Support%")
                ->first();
            ...
            if ($jobFamily && $jobFamily->jobTitles->isNotEmpty()) {
                ...
                // Fallback: try to find any GA Support job title
                ...
                if ($gaFamily && $gaFamily->jobTitles->isNotEmpty()) {
                    ...
                    if (!$jobTitle) {
                        $jobTitle = JobTitle::factory()->create(['job_title' => 'General Affairs Support']);
                    }
            ...
            $userData = [
                ...
                'email'             => 'gasupport.employee.'.fake()->userName()."@{$validDomains}",
